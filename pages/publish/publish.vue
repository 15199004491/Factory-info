	...
	<view class="page">
		...
		<view class="form-list">
			<view class="form-item">
				...
				<input class="form-input" v-model="form.title" :placeholder="activeTab === 'second' ? '例：阳光花园 3室2厅' : '例：阳光花园 3室2厅整租'" placeholder-class="form-placeholder" />
			</view>
			...
			<view class="form-item" v-if="activeTab === 'second'">
				<text class="form-label">年代</text>
				<input class="form-input" v-model="form.year" placeholder="例：2015年" placeholder-class="form-placeholder" />
			</view>

			<view class="form-item" v-if="activeTab === 'rent'">
			...
			<view class="form-item">
				<text class="form-label">{{ activeTab === 'second' ? '售价(万)' : '月租(元)' }}</text>
				<input class="form-input" v-model="form.price" type="digit" :placeholder="activeTab === 'second' ? '例：128' : '例：2800'" placeholder-class="form-placeholder" />
			</view>
		...
		methods: {
			...
			onLoad(options) {
				if (options && options.action === 'edit') {
					var type = options.type || 'second'
					var id = parseInt(options.id)
					this.activeTab = type
					this.editingId = id
					var key = type === 'second' ? 'published_second' : 'published_rent'
			...
			async onSubmit() {
				...
				if (this.editingId) {
					var idx = list.findIndex(function(i) { return i.id === this.editingId }.bind(this))
					if (idx > -1) {
